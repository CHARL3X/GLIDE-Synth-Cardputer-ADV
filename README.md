...
- **Or let the instrument find it: hold `fn`+`k` and it LISTENS.** The synth goes quiet while the mic hears whatever's playing in the room — in ~3-second rounds, stopping the moment it's sure and listening up to ~9 s when the song is being coy (a single round can land on one chord and name *its* key; more rounds hear the changes). A chromagram works out the song's key (root *and* major/minor), and the instrument retunes itself. If you're on one of the four vanilla scales (Major, Natural minor, the two pentatonics), it also matches the *mode*: an A-minor song under Major pent swaps you to Min pent with the root on A — home key = the song's actual tonic, not its relative twin. Exotic scales (Blues, Dorian, Hirajoshi…) are a flavor you chose on purpose, so those stay put and just take the relative root instead (a C-major song under Blues lands on A — the pentatonic-boxes trick, automated). The result card shows the twelve pitch-class bars it heard, the detected key, and the applied root; a weak or silent room says NO SIGNAL and changes nothing. Tap `fn`+`k` to nudge if it guessed a neighbour. (Cardputer ADV only — it needs the mic; no mic just means a visible "mic unavailable", never a broken instrument.)
