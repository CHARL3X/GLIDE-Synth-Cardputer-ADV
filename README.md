...
And **no two GLIDEs sound alike.** The sounds aren't a factory list you pick from — you roll them from a generative engine, evolve them, and keep the ones that hit. Every roll commits to a *character* first (a pluck, a bell, a pad, an acid squelch, a wobble bass…) then paints inside that character's bounds, so a fresh sound is always playable and never mid-everything mush. And the engine seeds two of your ten slots from a number only your device has: out of the box, before you've touched anything, your unit already sounds like no one else's on Earth. This is not an instrument where everyone who owns one sounds the same. That's the point.

Two more that earn their place up top:

- **Tilt morph.** The gyro is a performance controller. Lean the device forward and back and the sound *blends continuously* into a second one — or route tilt to vibrato, wah, or swell instead. One `enter` press to toggle, and it follows your hands across every sound.
- **It finds a song's key by ear.** Hold `fn`+`k`, the mic listens to whatever's playing in the room, works out the key — root *and* major/minor — and the instrument retunes itself so you can solo over anything. (ADV only — it needs the mic.)
...
GLIDE runs from the microSD card through **[bmorcelli's Launcher](https://github.com/bmorcelli/Launcher)**. You flash Launcher **once**; after that, GLIDE (and every update) is just a file you copy to the card.

1. **Flash Launcher first** — one time only. Follow the [Launcher instructions](https://github.com/bmorcelli/Launcher) (its web flasher is the easy way). If your Cardputer already runs Launcher, skip this.
2. **[⬇ Download the latest `GLIDE.bin`](https://github.com/CHARL3X/GLIDE-Synth-Cardputer-ADV/releases/latest/download/GLIDE.bin)** and copy it onto the microSD card, into the `/apps/` folder. (That link always points at the newest build; the [Releases page](https://github.com/CHARL3X/GLIDE-Synth-Cardputer-ADV/releases) has the version notes.)
3. **Boot → SD → GLIDE.** Done.
...
Two ways — and frankly, dragging the file is easier:

- **Drag the new bin.** [Download the latest `GLIDE.bin`](https://github.com/CHARL3X/GLIDE-Synth-Cardputer-ADV/releases/latest/download/GLIDE.bin) and copy it over the old one in `/apps/` on the card. That's the whole update. Your saved sounds, tweaks, and settings all survive — they live in flash outside the app.
- **OTA through Launcher.** Hit `esc` (or any key) on Launcher's start screen, go to **OTA**, connect to your WiFi, and find **GLIDE** in the list. Launcher downloads and installs the same latest build.
...
There's a full **HOW TO PLAY** screen on the device itself (settings → help), and the complete **[manual](docs/manual.md)** in this repo. But this is genuinely most of it:
...
- **Press keys.** It sounds good immediately — scale lock means every key is a scale tone, no dead notes. The rows are strings tuned a fourth apart, like a guitar.
...
- **Roll your own sounds.** `tab` → settings opens on two big **Randomize** and **Mutate** buttons. Every roll commits to a character (pluck, bell, pad, acid, organ, wobble bass…) and auditions instantly. Undo/redo means you never lose a keeper; `fn`+`shift`+letter saves it to a slot, or save it to SD with a name.
- **Tilt the device.** `enter` toggles the gyro: lean forward/back and the sound morphs into the one you were just on; left/right adds vibrato. Rewire either axis in settings.
...
Everything past that — tilt control, the looper's overdub stack, the auto-progression, the mod matrix, key detection by ear — is in the **[manual](docs/manual.md)**.
...
- **Idle dimming + screensaver.** Leave it sitting and the backlight eases down to save the panel and battery; leave it longer and a phosphor screensaver takes over — the string at rest: a standing wave held across the screen while a soft window of light slides slowly along it, the sound's name whispered underneath (and if a loop or jam is still running, the wave answers it). Any key wakes it *and* plays the note — the first touch is never wasted. Settings → *Screen idle* (off / dim / dim + saver).
