...
- **Idle dimming + screensaver.** Leave it sitting and the backlight eases down to save the panel and battery; leave it longer and a phosphor screensaver takes over — a resting oscilloscope tracing a slow, ever-morphing Lissajous, the sound's name whispered underneath (and if a loop or jam is still running, the figure breathes with it). Any key wakes it *and* plays the note — the first touch is never wasted. Settings → *Screen idle* (off / dim / dim + saver).
