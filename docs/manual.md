...
That flash is a small (~16 K) partition **shared with the Launcher and every other app on the card**, so it can fill up without GLIDE writing a byte. When it does, the first thing to fail is saving a sound over a slot — slot saves are the biggest single write GLIDE ever makes, so they die while settings keep persisting for a while longer. You'll never have to guess: the save error reads **storage full** and names the fix on the spot, settings → SYSTEM → *Storage* shows roughly how many saves still fit, and the next boot shows a red **STORAGE FULL** warning with the same fix. The instrument keeps playing throughout — nothing about *making* sound touches flash. The fix is the backspace reset below; save any sounds you care about to the SD card first (library → save), since slot contents don't survive it.
...
- **press and hold backspace during the boot splash:** full factory reset, settings and saved sounds, even if stored state ever wedges the UI. Hold it through the red confirm bar (~1.5 s); release at any point cancels. Deliberate on purpose, since a stray tap used to wipe people's sessions. It has to be a press made *during* the splash and then sustained; the ADV's keyboard chip is event-driven and can't see a key held from power-on. This reset is also the cure for a **STORAGE FULL** save error or boot warning: it first clears GLIDE's own keys, and if the shared partition still can't take a sound save after that, it escalates to erasing and rebuilding the whole partition (save your sounds to SD first — your two generative slots keep their identity, and the odometer rides across). Because that partition is shared, other apps' settings and the Launcher's saved Wi-Fi networks are cleared with it; they rebuild their defaults on next use.
