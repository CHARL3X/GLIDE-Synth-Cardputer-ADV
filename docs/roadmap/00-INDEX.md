...
**D2 — `` ` `` says "exit" but performs a restart.** `ESP.restart()` reboots into the same boot partition, which is GLIDE — the player lands back in the instrument, launcher never appears. Two honest resolutions, in order of ambition: (a) true exit — if M5Launcher is resident in another OTA slot on the ADV, `esp_ota_set_boot_partition` to it before restarting; needs the actual partition table read off hardware first (probe before building, doc-06 spirit), zero new gestures if it works. (b) if the launcher isn't bootable from flash, rename the gesture's hint from "exit" to "restart" and let the word match the deed. Either way the current label oversells.
